...
function renderHeatmap() {
    ...
    const year = 2026; // Sesuai tahun saat ini
    const months = [
        { name: 'Jan', days: 31 }, 
        { name: 'Feb', days: 28 }, // 2026 bukan kabisat
        { name: 'Mar', days: 31 },
        { name: 'Apr', days: 30 },
        { name: 'Mei', days: 31 }
    ];
    ...
    // 1. Label Hari (M S S R K J S)
    const dayLabels = document.createElement('div');
    dayLabels.style.display = 'grid';
    dayLabels.style.gridTemplateRows = 'repeat(7, 12px)'; // Tinggi baris 12px
    dayLabels.style.marginTop = '18px'; // Geser ke bawah agar sejajar kotak, bukan label bulan
    ...
    months.forEach((month, monthIndex) => {
        ...
        // Kunci lebar grup bulan agar tidak fleksibel melar
        ...
        // --- POIN KRUSIAL ---
        // repeat(7, 12px) = Mengunci 7 baris setinggi 12px
        daysGrid.style.gridTemplateRows = 'repeat(7, 12px)'; 
        // gridAutoColumns = Mengunci lebar setiap kolom baru sebesar 12px
        // Ini akan mencegah kolom terakhir melebar!
        ...
        for (let d = 1; d <= month.days; d++) {
            ...
            // Garis pembatas tipis agar kotak terlihat terpisah meski rapat
            cell.style.border = '1px solid rgba(255,255,255,0.4)'; 
            
            // Logika "Loncatan" Tanggal 1
            if (d === 1) {
                cell.style.gridRowStart = dayOfWeek + 1;
            }

            // Pewarnaan berdasarkan jumlah main
